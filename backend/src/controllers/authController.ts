...
export class AuthController {
  register = async (req: AuthedRequest, res: Response) => {
    ...
    if (existing > 0 && env.NODE_ENV !== 'test') {
      throw new AppError(403, 'El registro público está cerrado. Escribe a kojurebi@gmail.com', 'REGISTER_CLOSED');
    }
    const body = registerSchema.parse(req.body);
    const user = await userRepository.create({ ...body, role: 'admin' });
  ...
  login = async (req: AuthedRequest, res: Response) => {
    ...
    if (user.role !== 'admin') {
      throw new AppError(403, 'Esta cuenta no puede administrar la tienda', 'FORBIDDEN');
    }
    const token = signToken({ sub: user.id, email: user.email, role: user.role });
    setAuthCookie(res, token);
    res.json({ user });
